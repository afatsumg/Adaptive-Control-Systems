clear;clc;
%Periyodu belirleme
cycle=50;
t=0:1:800;
input = 1;

if(input == 1)%input kare dalga
    u=square(2*pi*0.01.*t,duty_cycle);
elseif (input ==2) %input t=50'de impulse
    u=1*(t==50);
end


%sistemdeki katsayilar
a=-0.8;
b=0.5;
c=-0.5;
gamma=0.01;

%parametreleri initialize ve preallocation ediyoruz
theta_hat(:,1)=zeros(2,1);
P_0=10^6*eye(2);
y(1)=0;

%Sistem cevabini elde etme
for i=2:800
    e(i)=sqrt(0.5)*randn(1);
    y(i)=-a*y(i-1)+b*u(i-1)+e(i)+c*e(i-1);
    
end

%SA Algoritmasi
for i=2:800
    phi_transpose(i,:)=[-y(i-1) u(i-1)];
    phi(:,i)=transpose(phi_transpose(i,:));
    P(:,i) = gamma/(phi_transpose(i,:)*phi(:,i));
    theta_hat(:,i)=theta_hat(:,i-1)+((P(:,i)*phi(:,i))*(y(i)-phi_transpose(i,:)*theta_hat(:,i-1)));
end
plot(theta_hat(1,:))
hold on
grid on
plot(theta_hat(2,:))
hold off